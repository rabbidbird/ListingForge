...
from .plans import ACTIVE_SUBSCRIPTION_STATUSES, PAID_PLANS, PLANS
...
def create_customer_portal_session(user_id: uuid.UUID) -> str:
    ...
    try:
        portal = stripe.billing_portal.Session.create(
            ...
            return_url=f"{settings.public_base_url}/About_Pricing",
        )
    except stripe.StripeError as exc:
...
def _apply_checkout(session, obj: dict[str, Any], event_created: int) -> dict[str, Any]:
    ...
    if obj.get("payment_status") not in {"paid", "no_payment_required"}:
        return {"updated": False, "reason": "checkout_not_paid"}
    user = session.get(User, user_id)
    if user is None:
    ...
    subscription.status = (
        "trialing" if obj.get("payment_status") == "no_payment_required" else "active"
    )
    ...
    return {"updated": True, "user_id": str(user_id), "plan": plan}
...
def _apply_subscription(
    ...
    subscription.stripe_event_created_at = event_created
    return {"updated": True, "user_id": str(subscription.user_id), "plan": plan}
...
def process_webhook_event(event: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        with session_scope() as session:
            ...
            detail: dict[str, Any] = {"updated": False}
            ...
            session.flush()
            return {"ok": True, "duplicate": False, "type": event_type, **detail}
