...
from core.utils import export_to_dataframe, get_listing_by_id
...
st.caption("Results are always drafts: review every supplied fact before publishing.")
st.caption(
...
if llm_ready:
    ...
    st.caption("SellerDrafts will structure only the facts you enter.")

stored = st.session_state.get("latest_single_draft")
has_latest_draft = bool(stored and stored.get("user_id") == str(user.id))
if st.session_state.pop("draft_created_notice", False) and has_latest_draft:
...
if has_latest_draft:
    ...
    result = get_listing_by_id(user.id, listing_id) or stored["result"]
    ...
    render_editable_draft(
        ...
        listing_id,
        result,
        key_prefix=f"single_{listing_id}",
    )

    st.divider()
    overall = result["scores"]["overall"]
    st.subheader(f"Draft checklist: {overall['status']}")
...
form_surface = (
    st.expander("Create another draft", expanded=False) if has_latest_draft else st.container()
)
with form_surface, st.form("listing_form", clear_on_submit=False):
    ...
    with st.expander("Optional search and product details", expanded=False):
        primary_keyword = st.text_input(
            ...
            placeholder="e.g. moon pendant necklace",
            help="Use a buyer phrase you selected. SellerDrafts does not research search volume.",
            max_chars=300,
        )
